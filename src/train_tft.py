...
def make_predictions(
    ...
    forecasts, index = model.predict(predict_loader, return_index=True)

    predictions = index.copy()
    predictions["forecast"] = forecasts.detach().cpu().numpy().reshape(-1)
...
def enrich_predictions(predictions: pd.DataFrame, prepared: DataPreparationResult) -> pd.DataFrame:
    ...
    enriched = enriched[["date", "model", "color", "size", "forecast", "time_idx"]]
...
def save_outputs(
    ...
    enriched_predictions.rename(columns={"forecast": "predicted_units"}, inplace=True)
    enriched_predictions.to_csv(output_path, index=False)

    summary = (
        enriched_predictions
        .groupby(["model", "color", "size"], as_index=False)["predicted_units"]
        .sum()
        .rename(columns={"predicted_units": "predicted_units_next_14_days"})
    )
...
def main() -> None:
    ...
    prepared = prepare_data(args.data, prediction_length=prediction_length)
